...
import com.huichongzi.fastwidget4android.utils.ThirdOrderBezierCurve;
 ...
 */
public class CurveFolioView extends View implements AnimationViewInterface{
    /**
     * 翻转时拉伸变形的最大值
     * 即翻转90度时，最外边长度的增加倍数
     */
    ...
    private float[] verts = new float[201 * 201 * 2];

    public CurveFolioView(Context context) {
        super(context);
        init();
    }

    public CurveFolioView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public CurveFolioView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public CurveFolioView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
...
//        }
         ...
         */
        mFolioBitmap = null;
        float w = topBitmap.getWidth();
        float h = topBitmap.getHeight();
        ThirdOrderBezierCurve thirdOrderBezierCurve;
        if (mFolioY >= getHeight() / 2) {
            //当翻转位置在中部偏下时，取topBitmapFolie，同时绘制区域为一个正梯形
            mFolioBitmap = topBitmapFolie;
            thirdOrderBezierCurve = new ThirdOrderBezierCurve(new float[]{0, h}, new float[]{0, h * 2 - h * rate}, new float[]{-w * rate / 2, h * 2 - h * rate});
            int index = 0;
            for(int i = 0; i < 201; i++){
                float fy = h / 200 * i;
                fy = h + fy * (1- rate);
                float[] p = thirdOrderBezierCurve.getPointBy01(new float[]{0, fy});
                float rateW = (w - p[0] * 2) / w;
                for(int j = 0; j < 201; j++){
                    float fx = w / 200 * j;
                    verts[index * 2] = p[0] + fx * rateW;
                    verts[index * 2 + 1] = p[1];
                    index++;
                }
            }
            ...
            thirdOrderBezierCurve = new ThirdOrderBezierCurve(new float[]{-w * rate / 2, h * rate}, new float[]{-w * rate / 2, h}, new float[]{0, h});
            int index = 0;
            for(int i = 0; i < 201; i++){
                float fy = h / 200 * i;
                fy = h * rate + h * (1 - rate) * fy / h;
                float[] p = thirdOrderBezierCurve.getPointBy01(new float[]{-w * rate / 2, fy});
                float rateW = (w - p[0] * 2) / w;
                for(int j = 0; j < 201; j++){
                    float fx = w / 200 * j;
                    verts[index * 2] = p[0] + fx * rateW;
        ...
        canvas.drawBitmapMesh(mFolioBitmap, 200, 200, verts, 0, null, 0, null);
