...
public class RecycleScrollViewActivity extends Activity {
	...
	private RecycleScrollListView list;
	private View bottom;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		list = (RecycleScrollListView) findViewById(R.id.recycle_scrollview_activity_list);
		...
		list.setOnSizeChangedListener(new RecycleScrollListView.OnSizeChangedListener() {
			@Override
		...
		list.setOnScrollListener(new RecycleScrollListView.OnScrollListener() {
			@Override
			...
			public void onScrollStateChanged(int state) {
				if (state == RecycleScrollListView.SCROLL_STATE_END) {
	...
	private synchronized void scrollChanged(int t) {
		// 计算当前展示的第一个item的postion和这个item的顶部与scrollview顶部的差值（既item不可见部分的高度）
		int position = t / itemSmallHeight;
		int offset = t % itemSmallHeight;
		int changeheight = (int) (offset * (RecycleScrollViewAdapter.ITEM_CONTENT_TEXT_SCALE - 1));

		// 减少当前展示的第一个item的高度。
		View first = list.getListContent().getChildAt(position);
		if (first == null) {
			return;
		}
		...
		if (position + 1 < adapter.getCount()) {
			...
			float scale = (float) offset / itemSmallHeight
			...
			float alpha = (RecycleScrollViewAdapter.ITEM_SHADE_DARK_ALPHA - RecycleScrollViewAdapter.ITEM_SHADE_LIGHT_ALPHA)
					* (1 - (float) offset / itemSmallHeight)
		...
		if (offset == 0 && position == adapter.getCount() - 1) {
		...
		if (t > itemSmallHeight * (adapter.getCount() - 1) - 20) {
